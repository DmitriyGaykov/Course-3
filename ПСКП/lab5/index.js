...
const app = createServer(async (req, res) => {
  ...
  req.on('end', async () => {
    try {
      if (url === '/send') {
        const result = await send('andrysa.shev.133@gmail.com', 'dima.gaykov.04@gmail.com', "nlqufrxmdukvxnbi", 'Hello mir, manera crutit mir')
      ...
      if (url === '/api/emails') {
        switch (method) {
          ...
          case 'POST':
            ...
            if (true) {
              const result = await send(recipient, sender, "nlqufrxmdukvxnbi", message);
