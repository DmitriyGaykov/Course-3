...
const app = createServer(async (req, res) => {
  try {
    ...
    if (req.headers.hasOwnProperty('content-type') && req.headers['content-type'].includes('multipart/form-data')) {
      const form = formidable({multiples: true});
      console.log('before parse')
      const [fields, files] = await form.parse(req);
      req.body = fields;
      req.emit('end', fields, files);
